...
/// Location of one MDD resource within the source file.
pub struct MddEntry {
    pub path: String,
    /// Absolute byte offset of the compressed block within the source .mdd file.
    pub file_offset: u64,
    pub block_csize: u32,
    pub block_dsize: u32,
...
/// Normalize an MDD resource path for consistent lookup.
///
/// Strips MDX URL schemes, converts backslashes to forward slashes,
/// lowercases, and removes the leading slash.
pub fn normalize_path(p: &str) -> String {
    let lower = p.replace('\\', "/").to_lowercase();
    let stripped = strip_scheme(&lower);
    stripped.trim_start_matches('/').to_string()
}

fn strip_scheme(s: &str) -> &str {
    for scheme in ["file://", "bword://", "sound://", "entry://"] {
